...
export async function detectJVMChangesFromPR(
  octokit: Octokit,
  owner: string,
  repo: string,
  prNumber: number,
  ...
  for (const file of files.filter(({filename}) => isJVMBuildFile(filename))) {
    try {
      changes.push(...(await analyzeJVMFile(octokit, owner, repo, prNumber, file.filename)))
    } catch (error) {
  ...
  try {
    ...
    if (isMavenFile(filename)) {
      return parseMavenChanges({
        filename,
        prNumber,
        loadContent: async (path, ref) => getFileContent(octokit, owner, repo, path, ref),
      })
    }
