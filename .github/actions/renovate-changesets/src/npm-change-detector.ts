...
export async function detectNPMChangesFromPR(
  octokit: Octokit,
  owner: string,
  repo: string,
  prNumber: number,
  ...
  for (const file of files.filter(candidate => isNPMFile(candidate.filename))) {
    changes.push(...(await analyzeFileChanges(octokit, owner, repo, prNumber, file)))
  }

  return deduplicateChanges(changes)
}

export async function detectNPMChangesFromFiles(
  workingDirectory: string,
  changedFiles: string[],
): Promise<DependencyChange[]> {
  const changes: DependencyChange[] = []
  for (const file of changedFiles.filter(candidate => isNPMFile(candidate))) {
    changes.push(...(await analyzeLocalFileChanges(path.resolve(workingDirectory, file), file)))
  }

  return deduplicateChanges(changes)
}

function isNPMFile(filename: string): boolean {
...
async function analyzeFileChanges(
  ...
  try {
    ...
    const baseContent = await getFileContent(octokit, owner, repo, prData.base.sha, file.filename)
    const headContent = await getFileContent(octokit, owner, repo, prData.head.sha, file.filename)
    return compareFileContents(file.filename, baseContent, headContent)
  } catch (error) {
...
async function analyzeLocalFileChanges(
  filePath: string,
  relativePath: string,
): Promise<DependencyChange[]> {
  try {
    return parseFileForDependencies(relativePath, await fs.readFile(filePath, 'utf8'))
  } catch (error) {
...
function compareFileContents(
  ...
  if (baseContent == null) return parseFileForDependencies(filename, headContent)
...
function parseFileForDependencies(filename: string, content: string): DependencyChange[] {
  try {
    if (filename.endsWith('package.json')) {
