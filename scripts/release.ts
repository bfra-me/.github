...
export async function updateInternalWorkflowPins(
  ...
  if (updatedFiles.length > 0) {
    ...
    await exec('git', ['push', '--force', '-u', 'origin', branch], {cwd})

    const body = `Auto-generated by release script.\n\nUpdated: ${updatedFiles.join(', ')}`
    ...
    const existingPr = existing.stdout.trim()
    ...
    if (existingPr === '') {
      ...
      await exec('gh', ['pr', 'edit', existingPr, '--body', body], {cwd})
    }

    await exec('gh', ['pr', 'merge', branch, '--auto', '--squash'], {cwd})
    console.log(
      `${existingPr === '' ? 'Created' : `Updated existing PR #${existingPr}`} and armed auto-merge for SHA pin updates: ${updatedFiles.join(', ')}`,
    )
  }
}
